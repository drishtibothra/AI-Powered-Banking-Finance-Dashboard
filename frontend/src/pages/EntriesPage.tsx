...
export default function EntriesPage() {
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id);
    try { await dispatch(removeEntry(id)).unwrap(); } finally { setDeletingId(null); }
  };
          ...
          {items.map((entry) => (
            <div key={entry.id} className="flex items-center justify-between px-5 py-4">
              ...
              <div className="flex items-center gap-4 shrink-0">
                ...
                <button onClick={() => handleDelete(entry.id)} disabled={deletingId === entry.id} aria-label="Delete entry"
