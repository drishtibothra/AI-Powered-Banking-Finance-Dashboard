...
from app.schemas.analytics import (
    SummaryResponse, TrendPoint, CategoryBreakdownItem, BurnRateResponse
)
...
def get_summary(
    ...
    if not summary:
        return SummaryResponse(
            ...
            total_income=0, total_expense=0, total_savings=0, net_free_balance=0,
        )
...
def get_category_breakdown(
    ...
    ).join(Entry, Entry.category_id == Category.id).filter(
        Entry.user_id == current_user.id,
        Entry.entry_type == entry_type.lower().strip(),
        func.extract("month", Entry.date) == month,
        func.extract("year", Entry.date) == year,
    ).group_by(Category.id, Category.name).order_by(func.sum(Entry.amount).desc()).all()
...
def get_burn_rate(
    ...
    if not summary:
        return BurnRateResponse(current_balance=0, average_daily_spend=0, days_remaining=None)
