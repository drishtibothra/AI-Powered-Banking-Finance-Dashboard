...
def get_transactions(
    ...
    query = db.query(Entry).filter(
        Entry.user_id == user_id, Entry.date >= start, Entry.date <= end
    )
...
def get_budget_status(
    db: Session, user_id: int, category_name: str, month: int, year: int
) -> dict:
    category = _resolve_category(db, user_id, category_name)
    if not category:
        return {"error": f"Category '{category_name}' not found"}
    ...
    percent_used = (
        round(float(spent / budget.limit_amount * 100), 1)
...
def calculate_affordability(
    db: Session, user_id: int, target_amount: float, target_date: str
) -> dict:
    target = date_type.fromisoformat(target_date)
    today = date_type.today()
    months_until = max(
        0, (target.year - today.year) * 12 + (target.month - today.month)
    )
...
def get_spending_trend(
    db: Session, user_id: int, category_name: str | None = None, months: int = 6
) -> dict:
    if category_name:
        category = _resolve_category(db, user_id, category_name)
        if not category:
        ...
        return {
            "category": category_name,
            "trend": [
                {"month": int(r.month), "year": int(r.year), "total": float(r.total)}
                for r in rows
            ],
        }
...
def create_budget(
    ...
    if category.entry_type != EntryType.expense:
        return {
            "error": f"Budgets can only be set on expense categories. '{category_name}' is type '{category.entry_type.value}'"
        }
...
def flag_anomaly(
    ...
    for cat in categories:
        ...
        hist_rows = (
            ...
            .filter(Entry.user_id == user_id, Entry.category_id == cat.id)
            .group_by("m", "y")
            .all()
        )
        ...
        past_totals = [
            float(r.total)
            for r in hist_rows
            if not (int(r.m) == month and int(r.y) == year)
        ]
        if not past_totals:
            continue
            ...
            anomalies.append(
                {
                    ...
                    "percent_increase": round(
                        ((float(current) - avg_past) / avg_past) * 100, 1
                    ),
                }
            )
    ...
    if not anomalies:
        return {
            ...
            "message": "No significant spending anomalies detected.",
        }
    return {"anomalies_found": True, "anomalies": anomalies}


def semantic_search_tool_wrapper(
    db: Session, user_id: int, query: str, top_k: int = 5
) -> dict:
        ...
        "results": [
            {
                ...
                "amount": float(e.amount),
                "date": str(e.date),
                "similarity_distance": round(float(d), 4),
            }
            for e, d in results
        ]
    }
